...
/// 用 FFmpeg 内置 mjpeg 编码器生成 kTestFrameCount 帧测试码流（MJPG 语义：
/// 每帧一个访问单元、皆关键帧、无参数集），模拟 UVC 双目相机输入。
/// mjpeg 编码器不可用时抛出 std::runtime_error（测试跳过）。
std::vector<common::EncodedFrame> EncodeMjpegTestFrames() {
    const AVCodec* encoder = avcodec_find_encoder_by_name("mjpeg");
    if (encoder == nullptr) {
        throw std::runtime_error("未找到 mjpeg 编码器，跳过 MJPG 解码测试");
    }
    ...
    for (int index = 0; index < kTestFrameCount; ++index) {
        ...
        while (avcodec_receive_packet(enc_ctx, packet) == 0) {
            if (packet->data != nullptr && packet->size > 0) {
                common::EncodedFrame encoded;
                encoded.codec = common::VideoCodec::kMjpeg;
                encoded.is_key_frame = true;  // MJPG 每帧独立
                encoded.data.resize(static_cast<std::size_t>(packet->size));
                std::memcpy(encoded.data.data(), packet->data,
...
TEST(VideoDecoderTest, DecodesMjpegToNv12Frames) {
    std::vector<common::EncodedFrame> encoded;
    try {
        encoded = EncodeMjpegTestFrames();
    } catch (const std::runtime_error& e) {
        GTEST_SKIP() << e.what();
    }
    ASSERT_FALSE(encoded.empty());
    ...
    config.width = kTestWidth;
    config.height = kTestHeight;
    config.prefer_hardware = false;  // 开发机软解路径（YUVJ420P 经 swscale 转 NV12）
    ...
    EXPECT_EQ(decoder.HardwareTransferLatency().total_count, 0u);
    EXPECT_EQ(decoder.RgaDmaTransferLatency().total_count, 0u);
    EXPECT_EQ(decoder.ActiveCodec(), common::VideoCodec::kMjpeg);
    EXPECT_FALSE(decoder.IsHardwareDecoder());
    EXPECT_EQ(decoder.EncodedFrameCount(), static_cast<std::uint64_t>(encoded.size()));
    EXPECT_GT(decoder.EncodedBytes(), 0u);
    EXPECT_EQ(decoder.KeyFrameCount(), static_cast<std::uint64_t>(encoded.size()));
