...
#include "control/tracking_types.h"
...
/// 测试基准配置：大限幅使限幅器近似透明，便于独立验证各通道数学。
/// 内参 fx=fy=500、主点居中 cx=cy=500：偏移 100 像素 → 视线角
/// atan(0.2) ≈ 11.3099°。
TrackingConfig MakeConfig() {
    ...
    cfg.camera.cx_px = 500.0;
    cfg.camera.cy_px = 500.0;
...
ControlSnapshot MakeSnapshot(std::int64_t now_ms) {
    ...
    s.track.center_x = 500.0;
    s.track.center_y = 500.0;
...
TEST(TrackingControlLawTest, RateModeMapsBetaToYawRate) {
    TrackingControlLaw law(MakeConfig());
    auto snap = MakeSnapshot(1000);
    snap.track.center_x = 600.0;
    snap.track.center_y = 500.0;  // 目标偏右 100 像素
    const auto out = law.Update(snap, 1000);
...
TEST(TrackingControlLawTest, RateModeClampsToYawRateLimit) {
    TrackingControlLaw law(MakeConfig());
    auto snap = MakeSnapshot(1000);
    snap.track.center_x = 950.0;
    snap.track.center_y = 500.0;  // 大偏移：β≈43.5° > 30°/s 上限
    const auto out = law.Update(snap, 1000);
...
TEST(TrackingControlLawTest, VerticalOffsetDrivesVz) {
    TrackingControlLaw law(MakeConfig());
    auto snap = MakeSnapshot(1000);
    snap.track.center_x = 500.0;
    snap.track.center_y = 600.0;  // 目标偏低 100 像素 → α>0 → 下降
    const auto out = law.Update(snap, 1000);
...
TEST(TrackingControlLawTest, VerticalVelocityClamped) {
    ...
    auto snap = MakeSnapshot(1000);
    snap.track.center_x = 500.0;
    snap.track.center_y = 990.0;  // α≈44.5° × 0.1 = 4.45 > 2 上限
    const auto out = law.Update(snap, 1000);
