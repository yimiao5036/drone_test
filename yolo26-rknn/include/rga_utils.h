...
// 自适应 letterbox 填充函数，使用 RGA 硬件加速，结果写入目标虚拟地址
int adaptive_letterbox(const cv::Mat& src, int target_size, uint8_t* dst_virtual_addr,
    letterbox_t* letterbox, uint8_t fill_color, int interpolation = INTER_LINEAR);

// 使用 OpenCV 实现的自适应 letterbox 填充（备用方案）
int CV_adaptive_letterbox(const cv::Mat& src, int target_size, uint8_t* dst_virtual_addr,
    uint8_t fill_color, int interpolation = cv::INTER_LINEAR);
