...
2. opset19重建基线已完成同时间段板端A/B：与生产模型性能差异约1.5%～3%，P95/P99基本等价，人工观察识别效果更好；在客观精度和长时验证前仍作为候选；
3. 未重训ReLU性能探针已完成板端正常模式测试：Y2平均13.57ms、P95 19.02ms、P99 21.20ms，完整处理平均16.57ms，YOLO队列平均5.98ms，入口到完成平均25.58ms；相对同时间段SiLU生产模型Y2约25.62ms、完整处理29.09ms，分别下降约47%和43%。逐层报告确认73个`ConvRelu`和14个`ConvReluAdd`均约33%/33%/33%分配到三核，二者合计5.208ms；旧模型87个`ConvExSwish`全部落Core0且为17.524ms，卷积激活块减少12.316ms。总OP时间从27.804降至11.908ms，`exSDPAttention`绝对时间仍约2.469ms并全部落Core0，现成为20.73%的最大剩余单项。该ReLU探针未重训，禁止用于精度结论和生产。
4. 激活/融合OP方向已从性能和核心分配两方面确认，正式ReLU模型正在训练；训练完成后必须先确认checkpoint中ReLU仍存在，再做独立精度、RKNN导出、长时稳定性和误检验证；
5. 仅开启`enable_flash_attention=True`构建候选，评估两个attention的约9%占比是否下降；
6. 单独构建opset11 baseline，判断ONNX opset是否改变RKNN图融合；
7. 再做“原始检测头输出、CPU解码”候选，评估约1ms级低风险收益；
8. 单独评估轻量化/移除attention，不能与激活修改混在一次实验；
9. 所有正式模型候选都使用相同视频和标注集比较mAP、远距离召回率、误检率、Y2平均/P95/P99；
10. 若只需要降低约12ms队列等待，而非单帧模型时间，另做双context绑定Core0/Core1的帧级并行探针，并丢弃乱序旧结果。
