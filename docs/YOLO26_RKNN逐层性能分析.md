...
3. 使用`export_relu_performance_probe.py`先生成未重训ReLU性能探针，只验证`ConvExSwish`是否消失、卷积是否三核分摊和Y2理论收益，禁止用于精度结论；
4. 若ReLU性能收益显著，清理数据泄漏和负样本后训练正式ReLU/ReLU6候选；若收益小则停止该方向；
