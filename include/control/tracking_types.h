...
// =============================================================================
// tracking_types.h —— 视觉跟踪控制律的中立输入/输出结构
//
// 对应规格：docs/物理追踪思路.md
//   §2   输入输出与数据流（控制意图枚举语义）
//   §6.4 与追踪链路的行为衔接（tracked / is_predicted 语义）
//
// 移植说明（2026-09-23，自 visual_track_control/vtc_types.h）：
//   - 原库 ControlSnapshot.track 内嵌 target_tracker::TrackResult；主工程
//     视觉链路走 VisualTargetObservation/VisualTargetStatus（不接入
//     target_tracker），故换成自有精简结构 VisualTrackSample；
//   - 全部为纯值类型（POD 风格），控制律热路径零堆分配；
...
/// 视觉追踪样本：主工程视觉链路（VisualTargetStatus）到控制律的适配类型。
struct VisualTrackSample {
    bool tracked = false;        ///< 当前帧是否有稳定跟踪目标（kLocked）
    bool is_predicted = false;   ///< 是否为预测（Coast）结果；主链路无预测，恒 false
    double center_x = 0.0;       ///< 目标中心像素 x（tracked=true 时有效）
    double center_y = 0.0;       ///< 目标中心像素 y
};
...
///
/// 超龄判定一律使用"取样时间 + 配置有效期阈值"（§7.1）。
struct ControlSnapshot {
    // ---- 视觉追踪结果（控制律主输入） ----
    VisualTrackSample track;
    ...
    // （§2.3）与位置式绝对航向合成；其余字段预留，由 FlightStateSnapshot 填充。
...
/// 与 MAV_FRAME_LOCAL_NED 一致；因此旋转后 vy_mps 可能非零（机体系侧向
/// 恒为 0，旋转不改变合速度大小，限幅在旋转前完成）。
