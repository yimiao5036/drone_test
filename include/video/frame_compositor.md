...
### 异步帧-检测对齐
消费线程 `Run` 循环：每轮先 `DrainDetections`（`TryTake` 清空检测订阅队列暂存到
`pending_`），再 `WaitTakeFor(100ms)` 取一帧，把这帧合成上去。即"取到一帧时应用此前
收到的最新检测"，天然适配检测结果晚于帧、或一次多目标的场景。
...
- 构建合成 NV12 帧 + 检测结果，验证 Start/Stop 幂等、输出标注帧有效、检测应用到帧、
  无效帧计错不发布、停机不再消费、重启后恢复。
- 像素正确性（框/文字位置、颜色）是视觉验收项，留待香橙派实机或人工出图检查（当前
  测试只验证数据链路与计数）。
