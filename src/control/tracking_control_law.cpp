...
//             tracking_types.h ControlMode 注释）
...
#include "control/frame_transform.h"
#include "control/staleness_checker.h"
...
TrackingControlLaw::TrackingControlLaw(const TrackingConfig& cfg)
    : cfg_(cfg),
      pixel_to_beta_(cfg.camera.fx_px, cfg.camera.cx_px),
      pixel_to_alpha_(cfg.camera.fy_px, cfg.camera.cy_px),
...
ControlOutput TrackingControlLaw::Update(const ControlSnapshot& snapshot,
    ...
    // ---- ④ 像素坐标 → 视线角（§4.1 水平 β / §5.1 垂直 α） ----
    // 内参式：偏移自主点（标定 cx/cy）量起，右正/下正；PixelToAngle 内部
    // 完成 (pixel − c)/f + atan。
    const double beta_deg = pixel_to_beta_.Convert(snapshot.track.center_x) * kRadToDeg;
    const double alpha_deg = pixel_to_alpha_.Convert(snapshot.track.center_y) * kRadToDeg;
